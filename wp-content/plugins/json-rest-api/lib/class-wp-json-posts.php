...
class WP_JSON_Posts {
	...
	/**
	 * author: tamnguyen
	 * desc: add attachment to post
	 */
	public function add_attachment_to_post($post_id, $image_url) {
	...
	protected function insert_post(array $data) {
		...
		// add tax to custom post type
		// wp_set_post_terms($post_ID, array(51, 26), array(51, 26), true);
		// wp_set_object_terms($post_ID, array(51, 26), 'tvshows_categories');
		...
		if (!empty($data['image_url'])) {
			// add_attachment_to_post($post_ID, $data['image_url']);
		}
		// End add tax, category
