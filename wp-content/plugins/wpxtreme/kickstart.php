 ...
 * @version    2.0.5
 * @since      1.0.0.b4
 * @note       You can found the latest version of this file on https://gist.github.com/gfazioli/da2e5ad7a5a7f2721b47
 *
 */
...
if ( !function_exists( 'wpxtreme_kickstart' ) ) {
   ...
   */
  function wpxtreme_kickstart( $sMainFile, $sMainClassName, $sMainClassFile, $sClassParent = 'WPXtreme' )
  {

    // Create function to boot plugin
    $fBoot = create_function( '', // Check all versions, and DO NOT PROCEED if some version is invalid
      'if( TRUE == wpxtreme_kickstart_check_environment( \'' . $sMainFile . '\' )) {' .
    ...
    if ( 'WPXtreme' != $sMainClassName ) {
      ...
      // See internal comment on wpxtreme_kickstart_not_loaded function
      $fCheckWpxtremeGlobalInstance = create_function( '',
        'if ( !isset( $GLOBALS[\'WPXtreme\'] ) ) {' . '$GLOBALS[\'wpxtreme_notice_refers_to\'] = \'' . $sMainFile .
        '\';' . 'add_action( \'admin_notices\', \'wpxtreme_kickstart_not_loaded\' );' . '}' );
...
// ---------------------------------------------------------------------------------------------------------------------
// Function wpxtreme_kickstart_styles
// ---------------------------------------------------------------------------------------------------------------------

if ( !function_exists( 'wpxtreme_kickstart_styles' ) ) {

  /**
   * Simple style for layouting
   *
   * @brief Kickstrat styles
   * @since 2.0.0
   */
  function wpxtreme_kickstart_styles()
  {
    ?><style type="text/css">.wpxm-kickstart-box #icon-plugins{display:none}.wpxm-kickstart-box{background-color:#7fa8cb;color:#fff;border:1px solid #7795b5;margin:40px 16px 16px 0;text-align:center;line-height:32px;-moz-border-radius:3px;-webkit-border-radius:3px;border-radius:3px}.wpxm-kickstart-box p{font-size:16px;font-weight:lighter;margin:0 32px 16px}.wpxm-kickstart-box h3,.wpxm-kickstart-box h2{color:#fff;text-shadow:0 1px rgba(0,0,0,0.2);font-weight:lighter;font-size:20px;margin:16px}.wpxm-kickstart-box h2{font-weight:bold;font-size:22px}.wpxm-kickstart-box strong{font-weight:bold}.wpxm-kickstart-box a,.wpxm-kickstart-box a:link,.wpxm-kickstart-box a:visited{color:#fff;text-decoration:none;text-shadow:none;border-bottom:2px solid #fff}.wpxm-kickstart-box a:hover,.wpxm-kickstart-box a:focus,.wpxm-kickstart-box a:active{color:rgba(0,0,0,0.5);border-bottom-color:rgba(0,0,0,0.5)}.wpxm-kickstart-box a.wpxm-kickstart-button,.wpxm-kickstart-box a.wpxm-kickstart-button:link,.wpxm-kickstart-box a.wpxm-kickstart-button:visited{background-color:rgba(0,50,100,0.3);border-radius:3px 3px 3px 3px;border:0;color:#fff;display:inline-block;font-size:14px;font-weight:bold;padding:12px;text-decoration:none;text-transform:uppercase}.wpxm-kickstart-box a.wpxm-kickstart-button:hover,.wpxm-kickstart-box a.wpxm-kickstart-button:focus,.wpxm-kickstart-box a.wpxm-kickstart-button:active{background-color:rgba(0,50,100,0.5)}.wpxm-kickstart-box small{font-size:12px;color:rgba(0,0,0,0.5)}</style><?php
  }
}

// ---------------------------------------------------------------------------------------------------------------------
// Function wpxtreme_kickstart_environment_notices
// ---------------------------------------------------------------------------------------------------------------------

if ( !function_exists( 'wpxtreme_kickstart_environment_notices' ) ) {
  ...
  function wpxtreme_kickstart_environment_notices()
  {
    ...
    wpxtreme_kickstart_styles();
    ?>
    <div class="wpxm-kickstart-box">
...
// ---------------------------------------------------------------------------------------------------------------------
// Function wpxtreme_kickstart_check_environment
// ---------------------------------------------------------------------------------------------------------------------

if ( !function_exists( 'wpxtreme_kickstart_check_environment' ) ) {
   ...
   * @return bool
   */
  function wpxtreme_kickstart_check_environment( $sMainFile )
  {
    ...
    if ( !empty( $aWPXMetadata['wpx_wpxtreme_min'] ) && defined( 'WPXTREME_VERSION' ) ) {
      ...
      if ( true == version_compare( $aWPXMetadata['wpx_wpxtreme_min'], WPXTREME_VERSION, '>' ) ) {
        ...
        if ( !isset( $GLOBALS['wpxtreme_notification_flag'] ) ) {
          add_action( 'admin_notices', 'wpxtreme_kickstart_environment_notices' );
...
// ---------------------------------------------------------------------------------------------------------------------
// Functions wpxtreme_kickstart_not_loaded
// ---------------------------------------------------------------------------------------------------------------------

if ( !function_exists( 'wpxtreme_kickstart_not_loaded' ) ) {
   ...
   * @brief Admin notice
   */
  function wpxtreme_kickstart_not_loaded()
  {
      ...
      if ( wp_verify_nonce( $_GET['_wpnonce'], 'wpxtreme' ) ) {
        ...
        if( !class_exists( 'WPXTREME_KICKSTART_INSTALLER_SKIN', false ) ) {
          ...
          final class WPXTREME_KICKSTART_INSTALLER_SKIN extends Plugin_Installer_Skin {
            ...
            {
              wpxtreme_kickstart_styles();
              echo '<div class="wpxm-kickstart-box">';
              parent::header();
            }
